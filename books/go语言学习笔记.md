...
## 单元测试
单元测试的注意事项：
* 如果需要setup/teardown机制，建议自定义一个名为TestMain函数，go test会改为执行该函数；
* 运行单元测试的时候，可以保存代码覆盖率等信息；
* 如果想要将单元测试结果放到文档里，建议改用example；

## 性能测试
性能测试注意事项：
* 性能测试除了可以测试cpu性能，还可以测试内存性能；
* 使用下列命令分析性能信息：```go tool pprof```；
* 如果需要排除额外操作的时间，可以使用timer的一些函数，例如reset/start/stop timer；

# 工具链

## 编译
编译的注意事项：
* 可以使用下列命令做预处理：```go generage```;
* 可以使用下列方法做条件编译：将平台和架构信息添加到主文件尾部；使用build编译指令；使用自定义tags指令；
